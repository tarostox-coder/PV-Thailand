...
## 8. เอกสารประกอบ (อ่านเพิ่มเมื่อเกี่ยวข้อง)

ก่อนแก้เรื่องไหน ให้เปิดอ่านไฟล์ที่เกี่ยวข้องก่อน:

| ไฟล์ | เปิดอ่านเมื่อ |
|------|--------------|
| `ARCHITECTURE.md` | จะเข้าใจ data flow / โครงสร้างภายใน / จุดเข้าของแต่ละ subsystem |
| `DATA-STRUCTURE.md` | จะแตะโมเดลข้อมูล, `FIELD_RULES`, สถานะ 2-Layer, ChangeLog schema |
| `DEBUGGING-LOG.md` | เจอบั๊ก/พฤติกรรมแปลก — เช็คก่อนว่าเคยเจอ+แก้ยังไง (อย่า regress) |
| `CHANGELOG.md` | อยากรู้ว่าฟีเจอร์ไหนมาเวอร์ชันไหน |
| `PV-DASHBOARD-HANDOFF.md` | ภาพรวมเร็ว, URL/env vars, ตารางจุดเข้าโค้ดของทุกฟีเจอร์ |
| `DEPLOY-SETUP.md` | ตั้งค่าปุ่ม Deploy (env var) |
| `apps-script/README.md` | ติดตั้ง onEdit logger (ใครแก้) |

> เมื่อแก้อะไรที่กระทบสิ่งเหล่านี้ ให้ **อัปเดตเอกสารที่เกี่ยวด้วย** —
> โดยเฉพาะเพิ่มรายการใน `DEBUGGING-LOG.md` (เมื่อแก้บั๊ก) และ `CHANGELOG.md` (เมื่อออกเวอร์ชัน)
